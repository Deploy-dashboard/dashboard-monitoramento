...
def criar_grafico_progresso(df_aux, ordem_tarefas, num_linhas):
    ...
    # Garante que Data é datetime antes de qualquer operação
    ...
    # Se não há dados, retorna figura vazia com mensagem
...
def atualizar_csv_geral(arquivo_somativa, arquivo_formativa, arquivo_fluencia, arquivo_correcao):
    ...
    flu = pd.read_csv(arquivo_fluencia)
    cor = pd.read_csv(arquivo_correcao)
...
def report_progresso(arquivo, tab_key, session_key, arquivo_tarefas):
    ...
    if session_key not in st.session_state:
        ...
        # Se o session_state tem DataFrame vazio mas o CSV já tem dados, recarrega
    ...
    if submitted:
        ...
        # Remove colunas sem nome geradas pelo data_editor
        ...
        # Limpa o session_state para forçar releitura do CSV atualizado
    ...
    if st.session_state[session_key].dropna(subset=['nome']).empty:
        st.info("Nenhum dado cadastrado ainda. Adicione subprogramas na tabela acima e salve para visualizar o progresso.")
        return

    df_aux = processar_tarefas(st.session_state[session_key], arquivo_tarefas)
    
    df = st.session_state[session_key].copy()
    df = df.dropna(subset=['subprograma', 'nome'])
